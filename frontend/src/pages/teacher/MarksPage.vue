<template>
  <div>
    ...
    <DataTable
      :value="marksStore.records"
      :loading="marksStore.loading"
      striped-rows
      removable-sort
      paginator
      :rows="20"
    ...
    <Dialog
      ...
      :header="editing ? 'Edit Mark' : 'Add Mark'"
      :style="{ width: '480px' }"
      modal
    >
      <div class="grid">
...
function openDialog(mark = null) {
  editing.value = mark;
  form.value = mark
    ? { ...mark, test_date: mark.test_date ? new Date(mark.test_date) : null }
    : defaultForm();
...
async function save() {
  ...
  const payload = {
    ...
    test_date: form.value.test_date
      ? new Date(form.value.test_date).toISOString().split('T')[0]
      : null,
  };
  try {
    if (editing.value) {
...
async function saveBulk() {
  ...
  const payload = {
    ...
    test_date: bulk.value.test_date
      ? new Date(bulk.value.test_date).toISOString().split('T')[0]
      : null,
    records: bulk.value.records
      .filter((r) => r.marks_obtained !== null)
      .map((r) => ({ student_id: r.id, marks_obtained: r.marks_obtained, remarks: r.remarks })),
  };
  try {
