<template>
  <div>
    <div class="flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
      ...
      <div class="flex gap-2 align-items-center flex-wrap">
        <IconField>
          ...
          <InputText v-model="search" placeholder="Search name / phone..." @input="debounceSearch" class="text-sm" />
        </IconField>
        <Select v-model="filterInstitute" :options="[{ id: null, name: 'All' }, ...instituteStore.institutes]"
                option-label="name" option-value="id" placeholder="Institute" class="text-sm" @change="load" />
    ...
    <DataTable :value="studentStore.students" :loading="studentStore.loading" striped-rows removable-sort
      ...
      <Column header="Portal">
        <template #body="{ data }">
          <Tag :value="data.user_id ? 'Registered' : 'No Portal'" :severity="data.user_id ? 'success' : 'secondary'" />
        </template>
      </Column>
      ...
      <Column header="Actions" style="width:120px">
        <template #body="{ data }">
          <div class="flex gap-1">
            <Button icon="pi pi-eye"   text rounded size="small" v-tooltip="'View'" @click="viewStudent(data)" />
    ...
    <!-- Create/Edit Dialog -->
    <Dialog v-model:visible="dialogVisible" :header="editing ? 'Edit Student' : 'Add Student'" :style="{ width: '600px' }" modal>
      <div class="grid">
        ...
        <div class="col-12 field" v-if="!editing">
          ...
          <MultiSelect v-model="form.batch_ids" :options="batchStore.batches"
                       option-label="name" option-value="id" class="w-full"
                       placeholder="Select batches" display="chip" />
        </div>
        <div class="col-12 field">
      ...
      <template #footer>
        ...
        <Button :label="editing ? 'Update' : 'Add'" icon="pi pi-check" :loading="saving" @click="save" />
      </template>
    </Dialog>

    <!-- View Dialog -->
    <Dialog v-model:visible="viewVisible" header="Student Details" :style="{ width: '560px' }" modal>
      <div v-if="viewData" class="flex flex-column gap-4">
        <div class="flex align-items-center gap-3">
          <Avatar :label="viewData.name?.[0]" shape="circle" size="xlarge" style="background:#3b82f6;color:white" />
          <div>
        ...
        <div class="grid text-sm">
          <div class="col-6"><span class="font-medium">Phone:</span> {{ viewData.phone }}</div>
          <div class="col-6"><span class="font-medium">Parent:</span> {{ viewData.parent_name }}</div>
          <div class="col-6"><span class="font-medium">Parent Phone:</span> {{ viewData.parent_phone }}</div>
          <div class="col-6"><span class="font-medium">QR Token:</span> <span class="text-xs text-color-secondary">{{ viewData.qr_token }}</span></div>
        </div>
        <div v-if="viewData.batches?.length">
          <div class="font-medium mb-2">Enrolled Batches</div>
          <div class="flex flex-wrap gap-2">
            <Tag v-for="b in viewData.batches" :key="b.id" :value="`${b.name} – ${b.institute_name}`" />
          </div>
        </div>
      </div>
    </Dialog>
...
function defaultForm() {
  return { name:'', email:'', phone:'', dob:null, gender:'', address:'', parent_name:'', parent_phone:'', parent_email:'', notes:'', batch_ids:[] };
}
...
async function save() {
  if (!form.value.name) return toast.add({ severity: 'warn', summary: 'Name is required', life: 2000 });
  saving.value = true;
  try {
    if (editing.value) {
    ...
    toast.add({ severity: 'success', summary: editing.value ? 'Updated' : 'Student added', life: 2000 });
