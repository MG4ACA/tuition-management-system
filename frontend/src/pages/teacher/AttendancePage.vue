    ...
    <div class="col-12 lg:col-5">
      <div class="surface-card border-round-xl p-4 border-1 surface-border">
        ...
        <div v-if="selectedBatch">
          ...
          <div
            id="qr-reader"
            class="w-full border-round overflow-hidden mb-3"
          ...
          >
            ...
            <div class="flex-1">
              <div class="font-bold text-lg">
                {{ lastResult.success ? lastResult.data?.name : 'Error' }}
              </div>
              <div class="text-color-secondary text-sm">{{ lastResult.message }}</div>
              <Tag
                v-if="lastResult.data?.fee_status === 'pending'"
                value="⚠ Fee Pending"
                severity="warn"
                class="mt-1"
              />
              <Tag
                v-if="lastResult.data?.fee_status === 'paid'"
                value="✓ Fee Paid"
                severity="success"
                class="mt-1"
              />
