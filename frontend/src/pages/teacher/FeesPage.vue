<template>
  <div>
    ...
    <div class="flex gap-3 mb-4 flex-wrap">
      <div
        v-for="s in feesSummary"
        :key="s.status"
        class="px-3 py-2 border-round-xl border-1 surface-border text-sm flex align-items-center gap-2"
      >
    ...
    <Dialog
      ...
      header="Update Fee Status"
      :style="{ width: '400px' }"
      modal
    >
      <div class="flex flex-column gap-3" v-if="updating">
        <div class="text-sm text-color-secondary">
          <strong>{{ updating.student_name }}</strong>
          – {{ updating.batch_name }} – {{ updating.month?.slice(0, 7) }}
        </div>
        <div class="field">
          <label class="block mb-1 font-medium">Status</label>
          <Select
            v-model="updateForm.status"
            :options="['paid', 'pending', 'partial', 'waived']"
            class="w-full"
          />
        </div>
        <div class="field">
    ...
    >
      <div class="flex flex-column gap-3">
        ...
        <Message severity="info">
          This will create "pending" fee records for all enrolled students in the batch.
        </Message>
      </div>
      <template #footer>
...
async function updateFee() {
  saving.value = true;
  try {
    await feesStore.update(updating.value.id, {
      ...updateForm.value,
      paid_at: updateForm.value.status === 'paid' ? new Date().toISOString() : null,
    });
