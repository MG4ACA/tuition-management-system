<template>
  <div>
    ...
    <!-- Avg Score Card -->
    <div class="surface-card border-round-xl p-4 border-1 surface-border mb-4 flex align-items-center gap-4">
    ...
    <div class="surface-card border-round-xl border-1 surface-border overflow-hidden">
      <DataTable :value="records" :loading="loading" paginator :rows="20" striped-rows class="p-datatable-sm"
                 sort-field="test_date" :sort-order="-1">
        ...
        <Column header="Score" sortable sort-field="percentage">
          <template #body="{ data }">
            <div class="flex align-items-center gap-2">
              <ProgressBar :value="data.percentage" style="width:80px;height:8px" :show-value="false"
...
const avg = computed(() => {
  ...
  return Math.round(records.value.reduce((s, r) => s + Number(r.percentage), 0) / records.value.length);
});

const avgClass = computed(() => avg.value >= 75 ? 'text-green-500' : avg.value >= 50 ? 'text-orange-500' : 'text-red-500');

const bands = computed(() => [
  { label: 'Excellent (≥85%)', count: records.value.filter(r => r.percentage >= 85).length, cls: 'text-blue-500' },
  { label: 'Good (75–84%)',    count: records.value.filter(r => r.percentage >= 75 && r.percentage < 85).length, cls: 'text-green-500' },
  { label: 'Pass (50–74%)',    count: records.value.filter(r => r.percentage >= 50 && r.percentage < 75).length, cls: 'text-orange-500' },
  { label: 'Below Pass (<50%)',count: records.value.filter(r => r.percentage < 50).length, cls: 'text-red-500' },
]);
