      ...
      <div class="col-12 md:col-4">
        <div class="surface-card border-round-xl p-4 border-1 surface-border text-center">
          <div
            ...
            :class="pendingFees > 0 ? 'text-orange-500' : 'text-green-500'"
          >
            {{ pendingFees > 0 ? `LKR ${pendingFees.toLocaleString()}` : '✓ Clear' }}
          </div>
          <div class="text-color-secondary mt-1">Fee Status</div>
          <Tag
            :value="pendingFees > 0 ? 'Fee Pending' : 'All Paid'"
            :severity="pendingFees > 0 ? 'warn' : 'success'"
            class="mt-2"
          />
        </div>
      </div>
    ...
    <div class="grid">
      <div class="col-12 lg:col-6">
        <div class="surface-card border-round-xl p-4 border-1 surface-border">
          <div class="flex justify-content-between align-items-center mb-3">
            ...
            <RouterLink to="/student/attendance" class="text-sm text-primary">
              View all →
            </RouterLink>
          </div>
          <div class="flex flex-column gap-2">
            <div
              v-for="rec in recentAttendance"
              :key="rec.id"
            ...
            >
              ...
              <Tag
                :value="rec.status"
                :severity="
                  rec.status === 'present' ? 'success' : rec.status === 'late' ? 'warn' : 'danger'
                "
              />
            </div>
            <div v-if="!recentAttendance.length" class="text-color-secondary text-sm">
              No records yet.
            </div>
          </div>
        </div>
      ...
      <div class="col-12 lg:col-6">
        <div class="surface-card border-round-xl p-4 border-1 surface-border">
          ...
          <div class="flex flex-column gap-2">
            <div
              v-for="mark in recentMarks.slice(0, 5)"
              :key="mark.id"
              class="flex align-items-center justify-content-between"
            >
              <div>
                ...
                <div class="text-xs text-color-secondary">
                  {{ mark.batch_name }} · {{ mark.test_date?.split('T')[0] }}
                </div>
              </div>
              <span
                class="font-bold"
            ...
            <div v-if="!recentMarks.length" class="text-color-secondary text-sm">
              No results yet.
            </div>
          </div>
        </div>
...
const marksAvg = computed(() => {
  ...
  const avg =
    recentMarks.value.reduce((sum, m) => sum + Number(m.percentage), 0) / recentMarks.value.length;
