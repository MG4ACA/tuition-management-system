<template>
  <div>
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      ...
      <Select v-model="selectedBatch" :options="batchOptions" option-label="label" option-value="value"
              placeholder="All Batches" class="w-13rem" show-clear />
      <Select v-model="selectedType" :options="typeOptions" option-label="label" option-value="value"
              placeholder="All Types" class="w-10rem" show-clear />
    </div>

    <ProgressSpinner v-if="loading" style="width:50px;height:50px" class="flex justify-content-center my-6" />

    <div v-else-if="filtered.length === 0" class="surface-card border-round-xl border-1 surface-border p-6 text-center text-color-secondary">
      No resources found.
    </div>

    <div v-else class="grid">
      <div v-for="res in filtered" :key="res.id" class="col-12 md:col-6 lg:col-4">
        <div class="surface-card border-round-xl border-1 surface-border p-4 h-full flex flex-column gap-2">
          ...
          <div v-if="res.description" class="text-color-secondary text-sm flex-1">{{ res.description }}</div>
          ...
          <div class="mt-auto pt-2">
            <a v-if="res.file_path" :href="`/uploads/${res.file_path}`" target="_blank" download
            ...
            <a v-else-if="res.link_url" :href="res.link_url" target="_blank" rel="noopener"
