...
exports.create = async (req, res) => {
  const { institute_id, name, subject, grade, day_of_week, time_slot, monthly_fee, max_students } = req.body;

  // Ownership check
  const [inst] = await pool.query('SELECT id FROM institutes WHERE id=? AND teacher_id=?', [institute_id, req.user.id]);
...
exports.update = async (req, res) => {
  const { name, subject, grade, day_of_week, time_slot, monthly_fee, max_students, is_active } = req.body;
  ...
  await pool.query(
    ...
    [name, subject, grade, day_of_week, time_slot, monthly_fee, max_students, is_active, req.params.id]
  );
