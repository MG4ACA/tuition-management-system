...
exports.getAll = async (req, res) => {
  const [rows] = await pool.query('SELECT * FROM institutes WHERE teacher_id = ? ORDER BY name', [
    req.user.id,
  ]);
...
exports.getOne = async (req, res) => {
  const [rows] = await pool.query('SELECT * FROM institutes WHERE id = ? AND teacher_id = ?', [
    req.params.id,
    req.user.id,
  ]);
...
exports.remove = async (req, res) => {
  const [result] = await pool.query('DELETE FROM institutes WHERE id = ? AND teacher_id = ?', [
    req.params.id,
    req.user.id,
  ]);
...
exports.summary = async (req, res) => {
  ...
  const [[inst]] = await pool.query('SELECT * FROM institutes WHERE id = ? AND teacher_id = ?', [
    id,
    req.user.id,
  ]);
